...
**每张图片下方必须添加文字描述**，说明图片中要表达的核心内容。这是质量检查的必要项（MC-23）。
...
> **图片说明**：{描述图片展示的核心内容，包含以下要素：}
> - 主题：{图片展示什么（架构/数据通路/状态机/时序）}
> - 关键元素：{模块名/信号名/状态名及其功能}
> - 流程：{数据流向/状态转移/时序逻辑}
> - 补充：{关键路径/延迟/特殊处理}
```
...
| 图片类型 | 描述要点 |
|----------|----------|
| 内部框图（`_arch`） | 模块列表、功能角色、数据流向、外部接口关系 |
| 数据通路（`_datapath`） | 阶段划分、每阶段处理操作、组合/时序属性、关键路径 |
| 状态机（`_fsm`） | 状态列表、转移条件、编码方式、非法状态回收 |
| 时序图（Wavedrom） | 信号组、握手逻辑、时序关系、关键延迟 |
| 接口端口图（`_intf`） | 输入/输出信号分组、位宽、功能概述 |

### 示例

```markdown
![input_if_mod 内部框图](wd_input_if_mod_arch.png)

> **图片说明**：本图展示 input_if_mod 的内部架构，包含 5 个核心子模块：
> - 描述符接收 FSM：控制描述符接收流程
> - sqinfo 查表：3cycle SRAM 读延迟
> - sqid→dqid 映射：全局 RR 指针按包级更新
> - 链表节点分配：从空闲链表取节点
> - payload 读取决策：btype 包判断
> 
> 数据从 data_in_if 接口流入，经查表+映射+分配后，写入 bd_cache_mod 的 SRAM，并行发送 RAM 读请求。
```
