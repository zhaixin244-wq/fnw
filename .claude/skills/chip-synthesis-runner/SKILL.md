...
description: 使用 oss-cad-suite yosys 对 RTL 代码执行综合，验证可综合性、输出面积估计和关键路径信息。
---
...
1. **前置条件**：chip-lint-checker 全部阶段通过
2. **确定输入**：顶层模块名、RTL 文件列表
3. **生成脚本**：在 `{module}_work/ds/run/` 下生成 `run_synth.sh`（模板见下方脚本模板）
