...
description: "Use when performing final self-check on RTL implementation before delivery. Triggers on '自检', 'self-check', 'self check', '实现自检', 'quality checklist', 'IC-01', 'IM-01', '检查清单'. Runs IC-01~39 + IM-01~08 implementation quality checklist."
---
...
0. **文件发现**：用 `Glob` 搜索 `{module}_work/ds/rtl/*.v` 获取 RTL 文件列表，确认 `.claude/shared/quality-checklist-impl.md` 存在。若文件不存在，暂停并提示用户
