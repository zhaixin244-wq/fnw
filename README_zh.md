...
#### 当 Agent 反馈多个问题时：逐一确认

Agent（尤其是评审 Agent）经常会一次性列出多个问题（Critical / Major / Minor）。**建议不要一次性全部接受，而是逐一确认**：
...
# Agent 会这样交互：
Clara：问题 #1（Critical）：LSR[0] 用了 !rx_fifo_full 而非 !rx_fifo_empty
       原因：Data Ready 应表示 FIFO 有数据可读，即 FIFO 非空
       修复方案：改为 !rx_fifo_empty
       是否确认修复？

用户：确认，继续下一个

Clara：问题 #2（Major）：IIR 编码中 BI/FE/PE 与 RxAvailable 冲突
       ...
```

**为什么逐一确认**：
- 每个问题的严重程度不同，你可能想先修 Critical 再看 Major
- 部分问题可能是误报，需要你判断是否真正需要修复
- 修复一个问题可能影响其他问题的上下文
- 避免 Agent 在你不了解的情况下修改了不该改的代码

**适用场景**：
- 架构评审报告（`chip-arch-reviewer` 输出的问题清单）
- RTL 代码审查发现的多个 lint 错误
- FS/UA 文档的一致性检查结果
- 任何 Agent 一次性列出 3 个以上修改建议时

#### 什么时候使用 Plan 模式

**Plan 模式**适用于需要先规划再执行的复杂任务。在 Claude Code 中，你可以通过以下方式进入 Plan 模式：
...
| 场景 | 原因 | 示例 |
|------|------|------|
| **新模块设计启动** | 需要先确认需求范围和方案方向 | "帮我设计一个 DDR 控制器" |
| **多模块集成** | 需要先理清模块间接口和依赖 | "把 UART、SPI、I2C 集成到外设总线" |
| **架构重构** | 需要先评估影响范围再动手 | "把 FIFO 从寄存器阵列改为 SRAM 实现" |
| **不确定的任务范围** | 需要先探索再决定做什么 | "优化一下这个模块的时序" |
| **跨阶段工作** | 涉及多个 Agent 协作 | "从需求到 RTL 全流程帮我做一遍" |
| **评审后修复** | 多个问题需要排优先级 | "评审报告里有 10 个问题，帮我规划修复顺序" |

**不建议使用 Plan 模式的场景**：

| 场景 | 原因 |
|------|------|
| 任务明确、范围小 | 直接执行更快，如"帮我改一下这个寄存器的复位值" |
| 已有计划、只需执行 | Agent 已输出待办清单，直接说"开始"即可 |
| 紧急修复 | 时间紧迫，直接改代码 |

**Plan 模式的工作方式**：

```
用户：先做个计划，我想把 UART 的波特率发生器从整数分频改为小数分频

Claude：我来分析一下影响范围并制定计划：

## 变更计划

### 影响范围
- uart_baud_gen.v — 核心修改（增加小数累加器）
- uart_top.v — 端口连接可能变化
- uart_FS_v1.0.md — PPA 指标更新
- uart_baud_gen_microarch_v1.0.md — 微架构文档更新

### 执行步骤
1. 更新 FS 文档中的波特率精度指标
2. 更新微架构文档中的数据通路设计
3. 修改 uart_baud_gen.v RTL 代码
4. 更新 uart_top.v 端口连接
5. 重新评审

### 风险
- 小数累加器增加约 20 个寄存器
- 需要验证波特率精度是否满足 < 0.1% 目标
